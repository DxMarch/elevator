defmodule Test.Single.DecisionTest do
  ...
  test "unknown floor, :down -> down,idle" do
    ...
    assert Transition.next_action(orders, state) == {:down, :idle}
  end

  test "no orders -> keep direction,idle" do
    orders = %{}
    state = %Elevator.FSM.State{floor: 1, direction: :up, between_floors: false}
    assert Transition.next_action(orders, state) == {:up, :idle}
  end

  test "nearest above -> up,moving" do
    orders = %{3 => MapSet.new([:cab]), 4 => MapSet.new([:hall_up])}
    state = %Elevator.FSM.State{floor: 1, direction: :up, between_floors: false}
    assert Transition.next_action(orders, state) == {:up, :moving}
  end
  ...
  test "nearest below -> down,moving" do
    ...
    assert Transition.next_action(orders, state) == {:down, :moving}
  end
  ...
  test "same floor cab while moving up -> up,door_open" do
    ...
    assert Transition.next_action(orders, state) == {:up, :door_open}
  end
  ...
  test "same floor hall_down while idle down -> down,door_open" do
    ...
    state = %Elevator.FSM.State{floor: 1, direction: :down, between_floors: false}
    assert Transition.next_action(orders, state) == {:down, :door_open}
  end
  ...
  test "same floor cab with orders above while moving up -> up,door_open" do
